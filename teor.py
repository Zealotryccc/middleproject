...
"""
ПОСЛЕ НАСТРОЕК СЕРВЕРА BASE.PY НАСТРАИВАЕМ DEVELOPMENT И testing.py
Нам требуется установить email в консоль
поставить очередеь задач
отлавливать логирование ошибок
использовать базу данных дял тестов 
отключать доступ к паролям для прохождения тестов
отключать кэш память в тестах
